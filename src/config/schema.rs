...
/// WeCom (WeChat Enterprise) channel configuration.
///
/// Supports two modes:
/// - **Bot Webhook** (legacy): set `webhook_key` only — send-only via bot webhook.
/// - **Enterprise App**: set `corp_id`, `corp_secret`, `agent_id`, `token`, and
///   `encoding_aes_key` — full bidirectional messaging with encrypted callbacks.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct WeComConfig {
    /// Webhook key from WeCom Bot configuration (bot webhook mode).
    #[serde(default)]
    pub webhook_key: Option<String>,
    /// Corp ID (enterprise ID) for Enterprise App mode.
    #[serde(default)]
    pub corp_id: Option<String>,
    /// App secret for the Enterprise App.
    #[serde(default)]
    pub corp_secret: Option<String>,
    /// Agent ID for the Enterprise App.
    #[serde(default)]
    pub agent_id: Option<i64>,
    /// Token for callback URL verification (Enterprise App mode).
    #[serde(default)]
    pub token: Option<String>,
    /// EncodingAESKey for message encryption/decryption (43-char base64, Enterprise App mode).
    #[serde(default)]
...
impl WeComConfig {
    /// Returns true if this is an Enterprise App configuration.
    pub fn is_enterprise_app(&self) -> bool {
        self.corp_id.is_some() && self.corp_secret.is_some() && self.agent_id.is_some()
    }
}
...
impl ChannelConfig for WeComConfig {
    ...
    fn desc() -> &'static str {
        "WeCom (Bot Webhook or Enterprise App)"
    }
}
...
impl Config {
    pub async fn load_or_init() -> Result<Self> {
        ...
        if config_path.exists() {
            ...
            if let Some(ref mut wc) = config.channels_config.wecom {
                if let Some(ref mut key) = wc.webhook_key {
                    decrypt_secret(&store, key, "config.channels_config.wecom.webhook_key")?;
                }
                if let Some(ref mut secret) = wc.corp_secret {
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        if let Some(ref mut wc) = config_to_save.channels_config.wecom {
            if let Some(ref mut key) = wc.webhook_key {
                encrypt_secret(&store, key, "config.channels_config.wecom.webhook_key")?;
            }
            if let Some(ref mut secret) = wc.corp_secret {
