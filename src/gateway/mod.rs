...
use crate::channels::{
    ...
    GmailPushChannel, LinqChannel, NextcloudTalkChannel, SendMessage, WatiChannel, WeComChannel,
    WhatsAppChannel,
};
...
fn wecom_memory_key(msg: &crate::channels::traits::ChannelMessage) -> String {
    format!("wecom_{}_{}", msg.sender, msg.id)
}
...
pub struct AppState {
    ...
    /// WeCom Enterprise App channel for callback webhook handling
...
pub async fn run_gateway(host: &str, port: u16, config: Config) -> Result<()> {
    ...
    // WeCom Enterprise App channel (if configured)
    let wecom_channel: Option<Arc<WeComChannel>> = config
        .channels_config
        .wecom
        .as_ref()
        .filter(|wc| wc.is_enterprise_app())
        .map(|wc| {
            Arc::new(
                WeComChannel::new_enterprise(
                ...
                .expect("Invalid WeCom EncodingAESKey"),
            )
        });
    ...
    if wecom_channel.is_some() {
        println!("  GET  {pfx}/wecom     — WeCom callback URL verification");
        println!("  POST {pfx}/wecom     — WeCom Enterprise App message webhook");
    }
    ...
    let inner = Router::new()
        ...
        .route("/wecom", get(handle_wecom_verify))
        .route("/wecom", post(handle_wecom_message))
...
/// GET /wecom — WeCom callback URL verification (echostr challenge)
async fn handle_wecom_verify(
    ...
    match wecom.verify_callback(&msg_signature, &timestamp, &nonce, &echostr) {
        Some(plaintext) => {
            tracing::info!("WeCom callback URL verified successfully");
            (StatusCode::OK, plaintext)
        }
        None => {
            tracing::warn!("WeCom callback verification failed");
            (StatusCode::FORBIDDEN, "Verification failed".to_string())
        }
    }
}

/// POST /wecom — incoming WeCom Enterprise App message
